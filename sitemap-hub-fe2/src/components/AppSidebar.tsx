    ...
    return (
      <div key={item.id} className="w-full">
        <Button
          ...
          className={`w-full justify-start hover:bg-accent/50 transition-colors group ${isActive ? 'bg-primary text-primary-foreground shadow-md' : ''
            }`}
          onClick={onClick}
        >
    ...
    return (
      <div className="w-64 h-full bg-card border-r border-border/50 backdrop-blur-sm flex flex-col">
        <div key="header" className="p-4 border-b border-border/50 flex items-center justify-between">
          <span className="text-sm font-medium">SitemapHub</span>
          {onClose && (
            <Button
              variant="ghost"
              size="sm"
