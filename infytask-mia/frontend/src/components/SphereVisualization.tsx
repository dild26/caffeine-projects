...
function SphereNode({ task, position, index, isAdmin, onEdit }: { 
    task: Task; 
    ...
    const scale = useMemo(() => {
        return 0.3 + (progressValue / 100) * 0.2;
    }, [progressValue]);
...
function SphereScene({ tasks, isAdmin, onEdit }: { tasks: Task[]; isAdmin: boolean; onEdit: (task: Task) => void }) {
    const positions = useMemo(() => {
        const radius = 4;
        const goldenAngle = Math.PI * (3 - Math.sqrt(5));
        
        return tasks.map((_, index) => {
            const y = 1 - (index / Math.max(tasks.length - 1, 1)) * 2;
            ...
            return [x, yPos, z] as [number, number, number];
        });
    }, [tasks]);
...
export function SphereVisualization({ project, tasks, isAdmin, resourceId, onCleanup }: SphereVisualizationProps) {
    ...
    if (tasks.length === 0) {
        return (
            <div className="flex h-[500px] items-center justify-center rounded-lg border border-dashed border-border">
                <p className="text-muted-foreground">No tasks to visualize. Create some tasks to see the 3D sphere!</p>
            </div>
        );
    }
        ...
        <div className="space-y-4">
            ...
            <div ref={canvasRef} className="h-[500px] w-full rounded-lg bg-gradient-to-br from-background to-muted/20">
                <Canvas camera={{ position: [8, 5, 8], fov: 50 }}>
                    <SphereScene tasks={tasks} isAdmin={isAdmin} onEdit={handleEdit} />
                </Canvas>
            </div>
        </div>
    );
