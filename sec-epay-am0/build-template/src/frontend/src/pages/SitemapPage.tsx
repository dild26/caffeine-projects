...
import type { SitemapEntry } from '../backend';
...
export default function SitemapPage() {
  ...
  const autoPages = allPages.filter(p => 'auto' in p.pageType);
  const manualPages = allPages.filter(p => 'manual' in p.pageType);
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {route.delegatedApp.length > 0 ? `Delegated to: ${route.delegatedApp[0]}` : 'Not delegated'}
                        </div>
                      </div>
                      <Badge variant={route.delegatedApp.length > 0 ? 'default' : 'secondary'}>
                        {route.delegatedApp.length > 0 ? 'Active' : 'Inactive'}
                      </Badge>
                    </div>
                    ...
                    <TableBody>
                      {sitemapState?.controlledRoutes.map((route) => (
                        <TableRow key={route.routeName}>
                          ...
                          <TableCell className="font-mono text-xs">
                            {route.delegatedApp.length > 0 ? route.delegatedApp[0] : <span className="text-muted-foreground">Not delegated</span>}
                          </TableCell>
                          <TableCell>
                            <Badge variant={route.delegatedApp.length > 0 ? 'default' : 'secondary'}>
                              {route.delegatedApp.length > 0 ? 'Active' : 'Inactive'}
