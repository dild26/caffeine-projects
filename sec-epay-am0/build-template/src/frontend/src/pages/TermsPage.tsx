...
import type { AdminNotice } from '../backend';
...
export default function TermsPage() {
  ...
  const getNoticeIcon = (notice: AdminNotice) => {
    if ('critical' in notice.noticeType) return <AlertTriangle className="h-5 w-5 text-destructive" />;
    if ('legal' in notice.noticeType) return <Scale className="h-5 w-5 text-warning" />;
    return <Info className="h-5 w-5 text-primary" />;
  };

  const getNoticeVariant = (notice: AdminNotice) => {
    if ('critical' in notice.noticeType) return 'destructive';
    return 'default';
  };
        ...
        {activeNotices && activeNotices.length > 0 && (
          <div className="mb-8 space-y-4">
            ...
            {activeNotices.map((notice) => (
              <Alert key={Number(notice.id)} variant={getNoticeVariant(notice)}>
