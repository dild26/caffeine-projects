...
export default function MainFormPage() {
  ...
  const [noticeForm, setNoticeForm] = useState<{
    title: string;
    body: string;
    noticeType: 'info' | 'legal' | 'critical';
    requiresAcceptance: boolean;
    linkedTermsVersionId: string;
  }>({
    title: '',
    body: '',
    noticeType: 'info',
  ...
  const handleCreateNotice = async (e: React.FormEvent) => {
    ...
    try {
      const newNotice: AdminNotice = {
        ...
        noticeType: { [noticeForm.noticeType]: null } as any,
        effectiveDate: BigInt(Date.now() * 1_000_000),
        requiresAcceptance: noticeForm.requiresAcceptance,
        linkedTermsVersionId: noticeForm.linkedTermsVersionId ? BigInt(noticeForm.linkedTermsVersionId) : null,
      };
                ...
                <form onSubmit={handleMainConfigSubmit} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Current: 1 USD = {adminSettings ? formatUnitValue(adminSettings.conversionRate) : '90'} INR
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Current: {adminSettings ? formatUnitValue(adminSettings.subscriptionFee) : '₹1,000'}
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleCreateNotice} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={(e) => setNoticeForm(prev => ({ ...prev, noticeType: e.target.value as any }))}
