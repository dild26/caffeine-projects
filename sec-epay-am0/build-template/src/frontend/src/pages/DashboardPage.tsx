...
import { fromBackendUnitValue, formatUnitValue } from '../lib/unitConversion';
...
export default function DashboardPage() {
  ...
  const totalPayIns = transactions
    .filter(t => 'payIn' in t.type_)
    .reduce((sum, t) => sum + fromBackendUnitValue(t.amount).value, 0);

  const totalPayOuts = transactions
    .filter(t => 'payOut' in t.type_)
    .reduce((sum, t) => sum + fromBackendUnitValue(t.amount).value, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {transactions.filter(t => 'payIn' in t.type_).length} deposits
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {transactions.filter(t => 'payOut' in t.type_).length} withdrawals
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                {recentTransactions.map((transaction) => (
                  <div key={Number(transaction.id)} className="flex items-center justify-between p-3 rounded-lg border border-border/50 hover:border-primary/50 transition-colors">
                    <div className="flex items-center space-x-3">
                      <div className={`rounded-full p-2 ${'payIn' in transaction.type_ ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                      ...
                      <div>
                        <p className="font-medium">
                          {'payIn' in transaction.type_ ? 'Pay-In' : 'Pay-Out'}
                        </p>
                    ...
                    <div className="text-right">
                      <p className={`font-semibold ${'payIn' in transaction.type_ ? 'text-green-600' : 'text-red-600'}`}>
                        {'payIn' in transaction.type_ ? '+' : '-'}
                        {formatUnitValue(transaction.amount)}
                      </p>
                      <Badge variant={'ok' in transaction.status ? 'default' : 'pending' in transaction.status ? 'secondary' : 'destructive'} className="text-xs">
