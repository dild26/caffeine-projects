...
export default function TransactionsPage() {
  ...
  const [currency, setCurrency] = useState<Variant_inr_usd>(Variant_inr_usd.inr);
  const [type, setType] = useState<Variant_payIn_payOut>(Variant_payIn_payOut.payIn);
  ...
  const formatCurrency = (amount: number, curr: Variant_inr_usd) => {
    return curr === Variant_inr_usd.inr ? `₹${(amount / 100).toLocaleString()}` : `$${(amount / 100).toLocaleString()}`;
  };
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={type} onValueChange={(v) => setType(v as Variant_payIn_payOut)}>
                  <SelectTrigger id="type">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={Variant_payIn_payOut.payIn}>Pay-In (Deposit)</SelectItem>
                    <SelectItem value={Variant_payIn_payOut.payOut}>Pay-Out (Withdrawal)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={currency} onValueChange={(v) => setCurrency(v as Variant_inr_usd)}>
                  <SelectTrigger id="currency">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={Variant_inr_usd.inr}>INR (₹)</SelectItem>
                    <SelectItem value={Variant_inr_usd.usd}>USD ($)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                      ...
                      <TableRow key={Number(transaction.id)}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            ...
                            <span>{transaction.type === Variant_payIn_payOut.payIn ? 'Pay-In' : 'Pay-Out'}</span>
                          </div>
                        </TableCell>
                        <TableCell className={transaction.type === Variant_payIn_payOut.payIn ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}>
                          {transaction.type === Variant_payIn_payOut.payIn ? '+' : '-'}
