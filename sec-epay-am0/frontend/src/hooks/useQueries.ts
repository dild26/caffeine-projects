...
export function useUpdateLeaderboard() {
  ...
  return useMutation({
    mutationFn: async ({ totalAmount, duration }: { totalAmount: bigint; duration: bigint }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useAcceptTerms() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['userTermsAcceptance'] });
    },
  });
}
...
export function useGetAllUserTermsAcceptances() {
  ...
  return useQuery<UserTermsAcceptance[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      // This would need a backend function to get all acceptances
      // For now, return empty array as the backend doesn't expose this yet
      return [];
    },
    enabled: !!actor && !isFetching,
  });
